...
def explore_kde(feature: str = Form(...)):
    ...
    try:
        kde = _st.gaussian_kde(s)
        xs = _np.linspace(float(s.min()), float(s.max()), 300)
        fig.add_trace(go.Scatter(x=xs, y=kde(xs), mode="lines",
                                 fill="tozeroy", line=dict(color="#10b981", width=2),
                                 fillcolor="rgba(16,185,129,0.15)"))
    except Exception:
        fig = px.histogram(S["df"], x=feature, histnorm="probability density",
                           color_discrete_sequence=["#10b981"])
...
def explore_mean_target(feature: str = Form(...)):
    ...
    if pd.api.types.is_numeric_dtype(sub[feature]):
        sub["_bin"] = pd.qcut(sub[feature], q=min(10, sub[feature].nunique()), duplicates="drop").astype(str)
...
def explore_facetgrid(feature: str = Form(...)):
    ...
    if n_classes > 8:
        # Too many classes; use color overlay instead of facets
        sub[tgt] = sub[tgt].astype(str)
